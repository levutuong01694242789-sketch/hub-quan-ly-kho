...
DEFAULT_FIFO_PATH = r'C:\Users\My ROG\Desktop\2_LOGISTICS_QUAN_LY_KHO\QUAN_LY_KHO_FIFO\KET_QUA_XUAT_KHO\KET_QUA_MOI_NHAT.xlsx'
OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_EXCEL_PATH = os.path.join(OUTPUT_DIR, 'KET_QUA_TRU_KHO_SAP_VDT.xlsx')
...
def load_master_bins(bin_file_path):
...
def process_fifo_deduction(fifo_file_path, master_bin_path=MASTER_BIN_PATH):
    """Process stock deduction from FIFO results file and map to SAP Storage Bins + Buffer Zones."""
        ...
        for r in range(5, ws_rem.max_row + 1):
            ...
            if sap_bin not in bins_db:
                if any(sap_bin.startswith(prefix) for prefix in ['TW_', 'VW_', 'VL_']):
                    # Dynamically create Buffer Bin Record
                    ...
                    bins_db[sap_bin] = {
                        'sap_code': sap_bin,
                        'old_code': s_old_bin,
                        'rack': 'BUFFER', # Special Rack category
                        'level': 0,
                        'pos_num': sap_bin,
                        'in_out': 'S', # S = Staging / Buffer
                        'old_name': s_old_bin,
                        'new_name': sap_bin,
                        'area': bin_area,
    ...
    print(f"📊 TỔNG QUAN XỬ LÝ:")
    print(f"  - Kệ Cao Tầng Master SAP (Dãy A-H): {len(rack_bins)} ô")
    print(f"  - Số ô kệ đang chứa Pallet: {occupied_rack_bins} ô ({occupancy_rate}%)")
    print(f"  - Số ô kệ trống khả dụng: {empty_rack_bins} ô")
    print(f"  - Vùng Đệm & Trung Chuyển (TW/VW/VL): {len(buffer_bins)} khu vực")
    print(f"  - Tổng hợp tồn kho toàn kho: {unit_totals_all}")
    ...
    print(f"✅ Đã xuất dữ liệu JSON cho Web App: {OUTPUT_JSON_PATH}")
...
def generate_excel_report(bins_db, picked_records, remaining_stock_records, output_path):
    ...
    ws_summary.title = "Tổng hợp Vị trí SAP"
    ...
    wb.save(output_path)
    print(f"✅ Đã tạo báo cáo Excel thành công: {output_path}")

if __name__ == '__main__':
    try:
        process_fifo_deduction(DEFAULT_FIFO_PATH)
